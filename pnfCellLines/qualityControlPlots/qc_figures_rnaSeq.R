...
outputPCA <- function(mat, annotes, fileName){
  # data needs to be centered and scaled in order to evaluate clustering 
  pca_res <- prcomp(t(mat), scale.=F, center = T)
...
# get annotations 
grch38.dat <- synapseQuery(paste('select * from entity where parentId=="', grch38.dir, '"', sep = ''))
genecode.dat <- synapseQuery(paste('select * from entity where parentId=="', gencode.dir ,'"', sep = ''))
annotes.grch38 <- grch38.dat[which(grch38.dat$entity.id %in% colnames(mat.tpm)),
                             c("entity.specimenID", "entity.id", "entity.nf1Genotype")]
...
# -----------------------------------------------------------------------
# check if files or annotations have been updated and the dimentions match 
# length(rep(rownames(mat.tpm), ncol(mat.tpm)))
# dim(res)
...
annotes.gencode <- genecode.dat[which(genecode.dat$entity.id %in% colnames(mat.tpm.gencode)),
                                c("entity.specimenID", "entity.id", "entity.nf1Genotype")]
...
# -----------------------------------------------------------------------
# check if files or annotations have been updated and the dimentions match 
# length(rep(rownames(mat.tpm.gencode), ncol(mat.tpm.gencode)))
# dim(res)
