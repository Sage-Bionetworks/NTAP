...
for(val in c("ZSCORE","TAUC","MAXR","LAC50")){
  ...
  for(qval in c(0.1,0.05)){

      sigs=rownames(tab)[which(tab$adj.P.Val<qval)]
      if(length(sigs)>2){
          sig.enrich=targetEnrichment(sigs)
      all.sigs[[val]]<-list(sigs)
          write.table(sig.enrich,file=paste(val,'EnrichedTargetsFrom_NoHetDistinctAcrossGenotype_q',qval,'.txt',sep=''),row.names=T,col.names=T)

    pheatmap(aucMat[sigs,],annotation_row=data.frame(Target=all.targs[sigs]),
           annotation_col=data.frame(Genotype=full.gt),cellwidth=10,cellheight=10,file=paste(val,'ValuesNoHetDistinctAcrossGenotype_q',qval,'.png',sep=''))
      }

          alt.sigs=rownames(tab)[which(alt.tab$adj.P.Val<qval)]
          if(length(alt.sigs)>2){
    alt.sig.enrich=targetEnrichment(alt.sigs)
    write.table(alt.sig.enrich,file=paste(val,'EnrichedTargetsFrom_WithHetDistinctAcrossGenotype_q',qval,'.txt',sep=''))
    het.sigs[[val]]<-list(alt.sigs)
    #write to file
    pheatmap(aucMat[alt.sigs,],annotation_row=data.frame(Target=all.targs[alt.sigs]),
             annotation_col=data.frame(Genotype=full.gt),cellwidth=10,cellheight=10,file=paste(val,'ValuesWithHetDistinctAcrossGenotype_q',qval,'.png',sep=''))
}
  }
}
...
for(f in filelist){
  synStore(File(f,parentId=pid),used=list(list(url='https://raw.githubusercontent.com/Sage-Bionetworks/NTAP/master/pnfCellLines/analysis/2016-05-03/differentialDrugTargetEnrichment.R',wasExecuted=T)))
